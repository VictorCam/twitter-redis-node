...
//comment.js 
let ncommentid = Joi.string().pattern(new RegExp(`^[a-zA-Z0-9-_]{${11},${length}}$`)).required().label("invalid ncommentid format")
let commentid = Joi.string().pattern(new RegExp(`^[a-zA-Z0-9-_]{${11},${length}}$`)).required().label("invalid commentid format")
let postid = Joi.string().pattern(new RegExp(`^[a-zA-Z0-9-_]{${11},${length}}$`)).required().label("invalid postid format")
let comment = Joi.string().min(1).max(1000).required().label("comment should be between 1 to 1000 characters")
let type = Joi.string().valid("like", "reg").required().label("invalid type (like/reg) allowed")
