...
slave_sql_dir = "../../cc_setup/sql/slave.sql"
server2_mariadb_ip = raw_input("what is the IP of your MASTER?: ")
replica_pass = raw_input("what is the password for user 'replica' in MASTER?: ")
logpos = raw_input("what is the position from master.py in MASTER?: ")
...
replace(slave_sql_dir, "root@mariadb", root_maria_pass)
replace(slave_sql_dir, "user@mariadb", user_maria_pass)
replace(slave_sql_dir, "master_IP@mariadb", server2_mariadb_ip)
replace(slave_sql_dir, "replica_pass@mariadb", replica_pass)
replace(slave_sql_dir, "logpos@mariadb", logpos)
...
os.system('mysql < ~/../cc_setup/sql/slave.sql')
